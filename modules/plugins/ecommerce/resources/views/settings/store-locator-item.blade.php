...
<div class="next-form-section">
    ...
    <div class="next-form-grid">
        <div class="next-form-grid-cell">
            ...
            <input type="text" class="next-input" name="state" id="store_state" value="{{ $locator ? $locator->state  : get_ecommerce_setting('store_state') }}">
        </div>
        <div class="next-form-grid-cell">
            <label class="text-title-field" for="store_city">{{ __('City') }}</label>
            <input type="text" class="next-input" name="city" id="store_city" value="{{ $locator ? $locator->city  : get_ecommerce_setting('store_city')}}">
        </div>
    </div>
    <div class="next-form-grid">
        <div class="next-form-grid-cell">
            ...
            <div class="ui-select-wrapper">
                <select name="store_country" class="ui-select" id="store_country">
                    @php
                    $selectCountry = $locator ? $locator->country : get_ecommerce_setting('store_country');
                    @endphp
                    @foreach(['' => __('Select country...')] + \EG\Base\Supports\Helper::countries() as $countryCode => $countryName)
                        <option value="{{ $countryCode }}" @if ($selectCountry == $countryCode) selected @endif>{{ $countryName }}</option>
                    @endforeach
                </select>
