...
unset($locales[$default['lang_locale']]);
@endphp
<script id="product_attribute_template" type="text/x-custom-template">
    <li data-id="__id__" class="clearfix">
        ...
        <div class="swatch-title">
            <input type="text" class="form-control" value="__title__" data-lang="{{ $default['lang_locale'] }}">
        </div>

        @foreach($locales as $locale)
            <div class="swatch-title">
                <input type="text" class="form-control" value="__title__" data-lang="{{ $locale['lang_locale'] }}">
            </div>
        @endforeach
...
<div class="swatches-container">
    <div class="header clearfix">
        ...
        <div class="swatch-title">
            {{ __('Title') }} @if($locales){!! language_flag($default['lang_flag'], $default['lang_name']) !!}@endif
        </div>

        @foreach($locales as $locale)
            <div class="swatch-title">
                {{ __('Title') }} {!! language_flag($locale['lang_flag'], $locale['lang_name']) !!}
            </div>
        @endforeach
        ...
        @if (count($attributes) > 0)
            @foreach($attributes as $attribute)
                <li data-id="{{ $attribute['id'] }}" class="clearfix">
                    ...
                    <div class="swatch-title">
                        <input type="text" class="form-control" value="{{ $attribute->getTranslation('title', $default['lang_locale']) }}" data-lang="{{ $default['lang_locale'] }}">
                    </div>

                    @foreach($locales as $locale)
                    <div class="swatch-title">
                        <input type="text" class="form-control" value="{{ $attribute->getTranslation('title', $locale['lang_locale']) }}" data-lang="{{ $locale['lang_locale'] }}">
                    </div>
                    @endforeach
