class ShippingManagement {
    init() {
            ...
            $.ajax({
                ...
                success: res => {
                    if (!res.error) {
                        ...
                        Falcon.showSuccess(res.message);
                    } else {
                        Falcon.showError(res.message);
                    }
                    _self.removeClass('button-loading');
                    $('#confirm-delete-region-item-modal').modal('hide');
                },
                error: error => {
                    Falcon.handleError(error);
                    _self.removeClass('button-loading');
                }
            });
        });
        ...
        $(document).on('keyup', '.input-sync-item', event => {
            ...
            $(event.currentTarget).closest('.input-shipping-sync-wrapper').find($(event.currentTarget).data('target')).text(Falcon.numberFormat(parseFloat(number), 2));
        });
        ...
        $(document).on('keyup', '.shipping-price-district', event => {
            ...
            _self.closest('tr').find('.support-shipping .rule-adjustment-price-item').text(Falcon.numberFormat(parseFloat(basePrice) + parseFloat(adjustmentPrice), 2));
        });
        ...
        $(document).on('keyup', '.base-price-rule-item', event => {
            ...
            $.each($(document).find('.support-shipping .rule-adjustment-price-item'), (index, item) => {
                ...
                $(item).text(Falcon.numberFormat(parseFloat(basePrice) + parseFloat(adjustmentPrice)), 2);
            });
        });
    }
}
