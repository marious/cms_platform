...
namespace EG\Language;

use EG\Base\Supports\Helper;
use EG\Language\Models\Language;
...
class LanguageManager
{
    ...
    public function handleMultiLangFields($data)
    {
        $locales = $this->getSupportedLocales();
        $translatedFields = [];
        if ($locales && count($locales)) {
            foreach ($locales as $key => $locale) {
                foreach ($data as $element => $value) {
                    if (strpos($element, $locale['lang_locale'] . '_') !== false) {
                        $elementWithOutLocale = str_replace($locale['lang_locale'].'_', '', $element);
                        $translatedFields[$locale['lang_locale']][$elementWithOutLocale] = $value;
                        if (isset($data[$elementWithOutLocale])) {
                            $translatedFields[$this->getDefaultLocale()][$elementWithOutLocale] = $data[$elementWithOutLocale];
                        }
                    }
                }
            }
