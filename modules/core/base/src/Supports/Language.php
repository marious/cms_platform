...
//        'ary'            => ['ar', 'ary', 'العربية المغربية', 'rtl', 'ma'],
    ...
    public static function getAvilableLanguageName($convertedToList = false)
    {
        $data = [];
        foreach (self::getAvailableLocales() as $lang) {
            $data[$lang['locale']]  = $lang['name'];
        }
        if ($convertedToList) {
            $results = [];
    ...
    public static function getAvailableLanguageValues()
    {
        $values = [];
        foreach (self::getAvailableLocales() as $lang) {
            $values[] = $lang['locale'];
