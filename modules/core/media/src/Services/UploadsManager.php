...
class UploadsManager
{
    ...
    public function createDirectory($folder)
    {
        ...
        if (Storage::exists($folder)) {
            return trans('media.folder_exists', compact('folder'));
        }
    ...
    public function deleteDirectory($folder)
    {
        ...
        if (!empty($filesFolders)) {
            return trans('media.directory_must_empty');
        }
    ...
    {
        if (!config('media.chunk.enabled') || !$file) {
            return Storage::put($path, $content);
        }

        $currentChunksPath = config('media.chunk.storage.chunks') . '/' . $file->getFilename();
        $disk = Storage::disk(config('media.chunk.storage.disk'));
